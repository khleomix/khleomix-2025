...
async function run() {
	...
	if (!url) {
		try {
			const wpResponse = await fetch('https://khleomix.local/wp-json', {
				agent: httpsAgent,
			});
			if (wpResponse.ok) {
				const data = await wpResponse.json();
				// Use the "home" property which contains the absolute URL
				url = data?.home || 'https://khleomix.local';
				console.log(`Detected site URL: ${url}`);
			} else {
				console.warn(
					'Failed to fetch site URL. Using default: https://khleomix.local'
				);
				url = 'https://khleomix.local';
			}
		} catch (error) {
