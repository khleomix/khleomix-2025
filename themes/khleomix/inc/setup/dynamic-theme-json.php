...
function init_dynamic_theme_json() {
	...
	// Add debug logging if WP_DEBUG is enabled and we're on the Khleomix Settings page.
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG && is_admin() ) {
		...
		add_action(
			...
			function () {
				...
				if ( $screen && 'tools_page_khleomix-settings' === $screen->id ) {
...
function debug_font_detection() {
	...
	<div class="notice notice-info is-dismissible" style="margin: 2em 0 1em;">
		<h3 style="margin-top: 1em;"><?php esc_html_e( 'Font Detection Debug', 'khleomix' ); ?></h3>
		<p>
			<strong><?php esc_html_e( 'Build fonts found:', 'khleomix' ); ?></strong> <?php echo count( $build_fonts ); ?><br>
			<strong><?php esc_html_e( 'Assets fonts found:', 'khleomix' ); ?></strong> <?php echo count( $assets_fonts ); ?>
		</p>
		<?php if ( ! empty( $font_families ) ) : ?>
			<p><strong><?php esc_html_e( 'Font families detected:', 'khleomix' ); ?></strong></p>
			<ul style="margin-left: 1.5em; margin-bottom: 1em;">
			<?php foreach ( $font_families as $family => $config ) : ?>
				<li>
					<?php echo esc_html( $family ); ?>: <?php echo count( $config['fontFace'] ); ?> <?php esc_html_e( 'variants', 'khleomix' ); ?>
		...
		<?php else : ?>
			<p><em><?php esc_html_e( 'No font families detected.', 'khleomix' ); ?></em></p>
		<?php endif; ?>
	</div>
	<?php
}
