...
function generate_font_preload( $generated_fonts, $output_file ) {
	...
	$php .= " */\n\n";
	$php .= "function khleomix_font_preload_links() {\n";
	$php .= "  \$preload_links = [\n";
...
function main() {
	...
	$source_fonts = scan_source_fonts( $khleomix_config['input_dir'] );

	if ( empty( $source_fonts ) ) {

		printf(
			"No source fonts found in %s\n",
			$khleomix_config['input_dir']
		);
		exit( 1 );
	}
	...
	$generated_fonts = generate_font_files( $source_fonts, $khleomix_config['output_dir'], $khleomix_config );
	...
	// Generate CSS.
	printf( "\nGenerating CSS...\n" );
	generate_font_css( $generated_fonts, $khleomix_config['css_output'] );

	// Generate preload links.
	printf( "\nGenerating preload links...\n" );
	generate_font_preload( $generated_fonts, $khleomix_config['preload_output'] );
	...
	printf(
		"  - Fonts: %s/\n",
		$khleomix_config['output_dir']
	);

	printf(
		"  - CSS: %s\n",
		$khleomix_config['css_output']
	);

	printf(
		"  - Preload: %s\n",
		$khleomix_config['preload_output']
	);
